...
def score_release(
    ...
    for token in WRONG_RECORDING_TOKENS:
        if token in name and token not in album_n:
            # A wrong-recording hit means this isn't the requested content at
            # all, no matter how well the title/seeders otherwise score — a
            # flat subtraction alone can still leave a well-matched,
            # well-seeded karaoke/tribute release above REJECT_CEILING and
            # eligible for auto-grab, so force it below the floor outright.
            score = min(score - 70.0, REJECT_CEILING - 1.0)
            break
